...
public class Game {
    ...
    //private final String[][] GAME_FIELD = new String[Gor][Ver];
    ...
    //allDecorFullField = [11]столбцов(слева направо)[11]строк(сверху вниз)
...
//        else {System.out.println("Вы где-то ошиблись, попробуйте ещё раз,."
//                + "\nВведите ЦИФРАМИ в консоль размеры поля, на котором хотите сыграть");

        else {System.out.println("Вы определённо где-то ошиблись при вводе. "
                    + "\nМы ещё работаем над нашим проектом..." +
                "\nА пока что введите любое число выслушайте какой вы жопорук!");
           readConcoleFieldSize();
        }
        return iVal;
...
//                 proverkaIndeksaStolbcov++, idemPoStolbcam++)
//        for (int idemPoStrokam = 0, proverkaIndeksaStrok = 2; idemPoStrokam < workField.length; proverkaIndeksaStrok++, idemPoStrokam++) {  //идём по строкам twoDimArray.length
//            for (int idemPoStolbcam = 0, proverkaIndeksaStolbcov = 2; proverkaIndeksaStolbcov< workField[idemPoStrokam].length;
//                 proverkaIndeksaStolbcov++, idemPoStolbcam++) {//идём по столбцам twoDimArray[i].length
////                try {
////                    Thread.sleep(1200);
////                } catch (InterruptedException e) {
////                    e.printStackTrace();
////                }
//                if(player.getMark().equals(workField[idemPoStrokam][proverkaIndeksaStolbcov])
//                        && player.getMark().equals(workField[idemPoStrokam][proverkaIndeksaStolbcov-1])
//                        &&player.getMark().equals(workField[idemPoStrokam][proverkaIndeksaStolbcov-2])){
//                    System.out.print("ТРИ В РЯД!"); //вывод элемента
//                }
//                if(player.getMark().equals(workField[proverkaIndeksaStrok][idemPoStolbcam])
//                        && player.getMark().equals(workField[proverkaIndeksaStrok-1][idemPoStolbcam])
//                        &&player.getMark().equals(workField[proverkaIndeksaStrok-2][idemPoStolbcam])){
//                    System.out.print("ТРИ В РЯД!"); //вывод элемента
//                }
//               //if()
//                System.out.print(" " + workField[idemPoStrokam][idemPoStolbcam] + " "); //вывод элемента
//            }
//            System.out.println();//перенос строки ради визуального сохранения табличной формы
//        }
